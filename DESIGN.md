...
```
 ...
 (core/llm.py) (tools/local_    (skills/skill_      (tools/mcp_       (后端选择/路径
      │         tools.py)        loader.py)          setup.py)        白名单/RAG 开关)
      │              │           get_rollback_tools()      │
      │  max_tokens  │  本地 6 个工具   get_git_tools()      │  npx 拉起
      │  重试/超时    │            (tools/rollback.py       │  filesystem server
      │              │             tools/git_tool.py)       │
      └──────┬───────┴────────┬────────┴─────────┬────────┘
             │                │                  │
             ▼                ▼                  ▼
   ...
   │  ├─ list_backups                 │  └─ git_command (add/commit/    │
   │  └─ (git_command 只读子命令)       │      push/pull/tag/...) ┘       │
   │                  │                │                    │           │
   │                  ▼                ▼                    ▼           │
   │           skills/*.md      rag/rag_tool.py →   tools/rollback.py    │
   │        （按需加载正文）      retriever→embedder   记录备份（见 §7）   │
   │                             (dense/bm25/hybrid)                     │
   └─────────────────────────────────┬───────────────────────────────────┘
                                      │
                        写入/编辑/删除类工具执行前
                                      ▼
                tools/confirm.py：confirm_action() 阻塞 input() 等 y/n
                          拒绝 → 直接返回"已被用户拒绝"，不执行
                                      │
                                      ▼
                       SessionLogger（core/logger.py）
...
`core/llm.py` 里，Snowflake / DashScope 分支的 `ChatOpenAI` 均设置
`max_tokens=16000`、`max_retries=3`、`timeout=120`，遇到瞬时 500/超时会自动重试，
避免大 payload 触发网关异常直接失败。Snowflake 分支会先尝试
`model_kwargs={"parallel_tool_calls": False}`，如果 Cortex 不支持则自动回退默认配置。
...
- 会话日志：每对话一个 `.txt`，由 `core/logger.py` 写入 `SINGLE_AGENT/logs/`（跟随
  代码目录，不再依赖 `~/Desktop`）。
